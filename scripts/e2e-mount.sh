...
# settings so the welcome notice is considered seen and the official DeepSeek
# route is the default model, so no setup modal is required to drive the UI.
# The route reads DEEPSEEK_API_KEY, exported below with a placeholder: the
# composer stays editable (an unreachable default model replaces the editor
# with a disabled placeholder) while keyless sends fail cleanly at API auth,
# which markers rely on to un-blank sessions without real model calls.
...
# Provide the credential the official DeepSeek route resolves through
