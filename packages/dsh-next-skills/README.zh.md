...
- **Skills** — 一个两列卡片网格，包含在项目与用户根目录中发现的所有技能
  （`.dsh/skills` 与 `.agents/skills`，分别针对当前工作区与全局扫描），
  以及每个提供方目录中的技能。搜索框、提供方筛选、仅显示已安装开关以及
  Show more 按钮（每页 30 张卡片）让大型目录依然流畅。同一个技能存在于
  多个根目录时，会为每个副本显示一张卡片，副本级操作因此清晰无歧义——
  每张卡片带有来源徽标（`project .agents`、`user .dsh` 等）、提供方标识，
  以及针对该副本的**删除**（可恢复）与**更新**（当同名目录技能存在差异时）
  按钮；**生效范围徽标**（`Everywhere`、`N workspaces` 或 `Off`）反映它的
  作用域。**Add/Manage** 按钮打开作用域弹窗：单选按钮决定该技能在哪里启用——
  Global（默认，所有工作区）或仅在勾选的已注册工作区中——安装或保存都会把
  该作用域写为纯配置。点击名称可以打开渲染为 markdown 的完整 SKILL.md。
...
技能文件；工作区的 `.agents/skills/`（或 `.dsh/skills/`）以只读方式扫描，
让手工创建、纳入版本控制的项目技能也能出现在网格中。
...
删除是可恢复的，且适用于所有副本：副本上的“删除”按钮会把该副本移入其
根目录的 `.trash` 目录（发现过程会跳过该目录），因此误删可以手动撤销。
任何 `.dsh`/`.agents` 副本都可以删除——不仅仅是插件安装的——当某个名称的
最后一个副本被删除时，来源记录与作用域条目会一并删除。名称已不再对应任何
副本或目录技能的作用域条目会被自动清理。
