 ...
 * truth for every dsh-next plugin's build. Emits a closure-factory artifact:
...
/**
 * The module specifiers the DSH shell freezes into its runtime module table.
 * This is the single source of truth for CLIENT_EXTERNALS below: a browser
 * bundle may only stay external (require from the loader table) for entries
 * listed here; anything else must inline, or it throws ERR_MODULE_NOT_FOUND
 * at boot. Keep in lockstep with the shell's `staticModules` seed table.
 */
 ...
 * PLATFORM_MODULES): the snapshot-store engine (createSnapshotStore/defineStore/
 * shallowEqual) lives in runtime pending its promotion-time rehoming, and
 * several importers ride this single exemption. At runtime the lazy CJS table
