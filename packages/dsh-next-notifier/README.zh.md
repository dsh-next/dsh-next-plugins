...
一个 DeepSeek Harness 插件：当 agent 完成回合、需要你的批准或向你提问时，显示
...
通知**仅使用浏览器（网页）通知**。当 DSH 页面打开（聚焦、后台或最小化）且
浏览器权限已授予时，宿主将事件排入队列，客户端通过页内点击打开处理器将其
显示。如果页面已关闭或缺少权限，该提醒将被丢弃。

每条通知的标题为 **emoji 图标 + 事件类型**（例如 "⚠️ Approval needed"、
"✅ Agent finished"），**正文为会话标题**（例如 "Design spec"），一眼即可
看出发生了什么、属于哪个会话。点击即可打开该会话。
