...
const pluginMarkers: Record<string, (page: Page) => Promise<void>> = {
  ...
  'dsh-next-worktrees': async (page) => {
    ...
    await conflictRow.screenshot({ path: join(shots, 'worktrees-conflict-icon.png') })
    ...
    await expect(mergeModal).toContainText('Fast-forward')
    await expect(page.locator('[data-dshx-button="merge"]')).toHaveText('Merge')
    await mergeModal.locator('.dshx-modal').screenshot({ path: join(shots, 'worktrees-conflict-merge-ff.png') })
    await page.locator('[data-dshx-button="merge"]').click()
    await expect(mergeModal).toContainText('Merged into', { timeout: 15_000 })
    await mergeModal.locator('.dshx-modal').screenshot({ path: join(shots, 'worktrees-conflict-merged.png') })
    await page.locator('[data-dshx-button="keep"]').click()
    await expect(mergeModal).toBeHidden({ timeout: 10_000 })
    expect(gitOk(workspaceA, ['merge-base', '--is-ancestor', `dsh-worktrees/${slug}`, 'HEAD'])).toBe(true)

    // Dirty worktree: Update still blocks. Merge lists the files as a
    // warning (this tree is already merged, so Merge stays disabled).
    ...
    await expect(page.locator('[data-dshx-dirty-files="dirty-primary"]')).toContainText('dirty-primary.txt')
    await expect(page.locator('[data-dshx-button="merge"]')).toBeEnabled()
    await page.locator('[data-dshx-button="merge"]').click()
    await expect(mergeModal).toContainText('Merged into', { timeout: 15_000 })
