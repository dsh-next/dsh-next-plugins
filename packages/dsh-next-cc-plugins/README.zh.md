...
- **浏览并安装插件** —— “插件”标签页以双列卡片网格列出所有市场中的每个
  ...
  过滤器和“仅已安装”开关。已安装的卡片显示其已安装版本，且只要市场携带有
  更新的版本，就会显示“更新”按钮（更新还会先重新同步该市场，因此总是拉取
  真正的最新版本）。每张卡片的“添加”（或“管理”）按钮会打开目标选择器：
  全局技能根目录与你的工作区的任意组合，一次安装完成。已持有该插件的目标
  会被锁定，只能各自卸载；“更新”会刷新每个目标。技能按所选目标分别落地；
  MCP 服务器、代理行、命令和钩子是插件级的，无论目标数量多少都只激活一次
  （弹窗中会说明这一点）。点击插件名称会打开**详情弹窗**：元数据、完整的
  组件清单（包括此桥接不安装的家族）、声明的依赖项，以及持久化在记录上的
  安装说明。目标特性引入之前的注册表记录会在读取时迁移到新结构。
- **安装插件** —— 每个插件的组件会落到原生消费它的 DSH 界面上：

  | Claude Code 组件 | DSH 目的地 | 激活时机 |
  | --- | --- | --- |
  | `skills/*/SKILL.md` | DSH 技能根目录（全局为 `~/.agents/skills`，每个工作区为 `<workspace>/.agents/skills`） | 立即，通过文件系统提供者的 watcher |
...
- **管理安装** —— 从上游更新已安装的插件（技能重新复制，被移除的技能可恢复
  地移入回收站，受管理的行以稳定的服务器/工具名重新渲染）并卸载它（技能移到
  根目录的 `.trash`，受管理的行和物化的插件副本退出）。物化的插件副本在重写
  时**保留 `node_modules`**（与 Claude Code 跨插件版本的做法相同），因此其
  MCP 服务器或钩子安装过依赖的插件在更新后依然可用；`package.json` 发生变化
  会被记录，以便插件自身的依赖引导程序刷新它们，而卸载会清除一切。安装与
  更新说明（未桥接的家族、被重命名的服务器、未解析的模板、依赖要求、技能
  frontmatter 差异）会持久化在安装记录上：卡片带有一个“安装说明”角标，
  悬停即显示列表，详情弹窗则完整展示。
...
- **MCP 模板展开** —— 服务器定义中的 `${CLAUDE_PLUGIN_ROOT}`、
  ...
  按原样保留并附一条说明（它在各安装目标之间没有单一取值），对未设置变量的
...
- 引用**插件级目录**（`references/`、`assets/` 等 —— 位于 `skills/` 旁边而非
  技能内部的文件）的技能在安装时会附带说明：Claude Code 从插件根目录运行
  技能，因此那些链接在那里可以解析，但 DSH 会把每个技能独立安装到技能根
  目录，被引用的路径无法解析。完整的插件副本会保持物化在
  `$DSH_HOME/cc-plugins/plugins/` 下以供钩子命令使用；需要这些文件的技能正文
  必须从那里读取。
...
市场、已安装的插件和模型映射会被镜像到 DSH 用户设置文档
...
工作区目标只携带文件夹名称 —— 绝对路径在每台机器上都不同。导入时，每个名称
会对照该机器的工作区注册表解析（文件夹匹配的已注册工作区；含糊或未知的名称
会跳过并留一条日志说明），而绝对路径（`workspace:/abs/path`）在手写且本地存在
时仍然有效。无法满足文件中某一部分的机器会保留其余一切 —— 市场和 `global`
目标仍会导入 —— 并且“插件”标签页会显示哪些导入被跳过及原因
（`cc-import-skipped`），因此缺失的部分会通过“添加”弹窗有意地安装，而不是靠猜。

面板的每次变更都会写入该小节（安装只记录存在性 —— 版本跟随上游）。在启动时，
以及每当该文档在磁盘上发生变化（设置提供者会热发布外部编辑），插件都会采纳
文档携带而本机缺失的内容：缺失的市场会被添加，缺失的插件会安装到其记录的
目标中（工作区目标仅当路径在本地存在时），模型映射在本机未保存任何映射时被
采纳。移除从不会被推断 —— 卸载始终通过面板显式进行。因此共享同一份
`settings.yaml` 即可在全新机器上尽力复现整套配置，并有日志记录。
...
含元数据、完整组件清单、依赖项和说明的详情弹窗；添加/管理打开多目标选择
弹窗）、**市场**（添加/刷新/移除来源，并显示各来源上次同步的时间；超过
24 小时的快照在面板打开时重新同步）和**模型**（把 Claude 模型名映射到此运行
时提供的模型上，实时发现 —— 未映射的名称继承会话的模型）。
