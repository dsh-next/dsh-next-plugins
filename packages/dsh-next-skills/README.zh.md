...
- **Skills** — 一个两列卡片网格，包含在项目与用户根目录中发现的所有技能
  ...
  多个根目录时，会按**名称**显示一张卡片，每个位置对应一行副本——每个副本
  带有来源徽标（`project .agents`、`user .dsh` 等）、绝对路径，以及针对该
  副本的**删除**（可恢复）与**更新**（当同名目录技能存在差异时）按钮；
  名称级别的**生效范围徽标**（`Everywhere`、`N workspaces` 或 `Off`）反映
  它的作用域。筛选行中的**全部更新**按钮显示可更新的副本数量，为零时置灰，
  并逐个更新它们。**Add/Manage** 按钮打开作用域弹窗：单选按钮决定该技能
  在哪里启用——Global（默认，所有工作区）或仅在勾选的已注册工作区中——
  安装或保存都会把该作用域写为纯配置。点击名称可以打开渲染为 markdown 的
  完整 SKILL.md。
