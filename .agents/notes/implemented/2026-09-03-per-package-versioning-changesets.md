...
cc-plugins is under active development and must not publish yet: its
manifest is marked `"private": true` (the single source of truth — npm
refuses a private publish and changesets skips it, so even a raw
`pnpm publish -r` cannot ship it). Verified empirically that a mixed
changeset (private + released package) makes `changeset version` fail and
that a private-only changeset produces no bump, so the gate now rejects
change files naming a private package and does NOT require one for
private-package source changes (tests in `verify-changeset.test.mjs`).
A real `changeset version` run bumped only skills and left cc-plugins at
0.1.0. Remove `"private": true` when ready.
