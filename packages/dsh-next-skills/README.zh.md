...
- **Skills** — 一个两列卡片网格，包含发现的每一个技能（项目、自定义与
  ...
  提供方徽标。筛选行中的**全部更新**按钮会显示可更新的技能数量，为零时
  置灰，并逐个更新它们。**Add/Manage** 按钮会打开作用域弹窗：单选按钮决定该技能
...
可读、可手工编辑、便于在开发者之间共享。这一小节是管理插件状态的唯一来源：
一个提供方存在，是因为这一小节列出了它；一个技能受插件管理，是因为
这一小节记录了它——绝不是因为某个缓存文件或清单旁车（两者都只是副本：
`$DSH_HOME/skills-market/` 下的提供方目录缓存和每个技能的
`.dsh-next-provider.json` 仅作参考，没有设置记录的缓存条目对面板而言
并不存在）。在提供方缓存同步之后，设置中有记录但文件缺失的技能会从
缓存重新安装，因此把设置小节复制给同事（或新机器）即可复现同一套技能：
提供方立即配置生效，首次启动会同步缓存并安装已记录的技能，作用域原样
生效（它们是文件夹名）。每次“全部刷新”都会以同样的重装收尾，因此首次
启动时同步失败的提供方，在下一次刷新时会自我修复。
...
清单（`.dsh-next-provider.json`）与设置记录并存。提供方可以暴露任意
数量的技能——没有数量上限（网格分页浏览，同步按内容哈希增量进行），
因此即使包含数百个技能的仓库也能正常同步和浏览。
