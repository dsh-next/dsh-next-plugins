/**
 * Host loader entry for the git plugin — runs in the DSH host process.
 *
 * The host half is a cordis plugin loaded from the profile composition via
 * the row in cordis.patch.yml (id git). Most GUI plugins have no host
...
export function apply(ctx: Context): void {
  // TODO(git): host-side behavior, e.g.
  //   ctx.systemPrompt.section({ name: 'plugin:git', order: 200, text: '...' })
